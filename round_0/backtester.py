...
class Backtester:
    ...
    def run(self):
        ...
        for timestamp, group in timestamp_group_md:
            ...
            state = self._construct_trading_state(
                traderData, timestamp, self.listings, order_depths,
                dict(own_trades), dict(market_trades), self.current_position, observations
            )
            ...
            for product in products:
                ...
                for order in orders.get(product, []):
                    executed_orders = self._execute_order(
                        ...
                        self.current_position, self.cash, trade_history_dict, sandboxLog
                    )
    ...
    def _add_trades(self, own_trades, market_trades):
        products = set(own_trades.keys()) | set(market_trades.keys())
        for product in products:
            self.trades.extend([self._trade_to_dict(t) for t in own_trades.get(product, [])])
        for product in products:
            self.trades.extend([self._trade_to_dict(t) for t in market_trades.get(product, [])])
    ...
    def _construct_trading_state(self, traderData, timestamp, listings, order_depths,
        ...
        return TradingState(traderData, timestamp, listings, order_depths,
                            own_trades, market_trades, position, observations)
    ...
    def _execute_buy_order(self, timestamp, order, order_depths, position, cash, trade_history_dict, sandboxLog):
        ...
        for price, volume in list(order_depth.sell_orders.items()):
            ...
            if abs(trade_volume + position[order.symbol]) <= int(self.position_limit[order.symbol]):
                ...
                sandboxLog += f"\nOrders for {order.symbol} exceeded limit of {self.position_limit[order.symbol]}"
            ...
            if trade.symbol == order.symbol and trade.price < order.price:
                ...
                if trade_volume == abs(trade.quantity):
                    ...
                    new_trades_at_timestamp.append(
                        Trade(order.symbol, order.price, trade.quantity - trade_volume, "", "", timestamp))
                    continue
    ...
    def _mark_pnl(self, cash, position, order_depths, pnl, product):
        ...
        if product in self.fair_marks:
            fair = self.fair_marks[product](order_depth)
        if fair is None:
            pnl[product] = None
        else:
